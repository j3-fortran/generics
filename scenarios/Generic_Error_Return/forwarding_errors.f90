...
contains
    function from_error(error) result(fallible)
    ...
    function from_answer(answer) result(fallible)
    ...
    function and_then_2_args(func, maybe_a, maybe_b) result(maybe_answer)
        abstract interface
        ...
        type(fallible_t<err, T>), intent(in) :: maybe_a
        type(fallible_t<err, U>), intent(in) :: maybe_b
        type(fallible_t<err, res>) :: maybe_answer
...
program example
    ...
    use fallible_m, only: fallible_t

    associate(maybe => &
            and_then_2_args( & ! This forwards the error along
...
contains
    function divide(numerator, denomenator) result(maybe_quotient)
        ...
        type(fallible_t<error_list_t, integer>) :: maybe_quotient

        if (denomenator == 0) then
            maybe_quotient = fallible_t(error_list_t("tried to divide by zero"))
        else
    ...
    function repeat_with_limit(string, repetitions, limit) result(maybe_string)
        ...
        type(fallible_t<error_list_t, string_t>) :: maybe_string

        if (len(string) * repetitions > limit) then
            maybe_string = fallible_t(error_list_t("tried to make string too long"))
        else
    ...
    function something_else_error_prone(num, string) result(maybe_thing)
        ...
        type(string_t), intent(in) :: string
        type(fallible_t<error_list_t, thing_t>) :: maybe_thing

        if (invalid_input(num, string)) then
            maybe_thing = fallible_t(error_list_t("invalid for some reason"))
        else
